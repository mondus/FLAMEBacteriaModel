...
# If the user wants cycles then should use the -c flag
# Ig the user wants that the name of the agent not be appended to the states names, he should use the -n flag
# Optionally, if the user sets the flaf -f the script outputs a source code skeleton of the functions used on the Model
#
#---------------------------------------------------------------------
#
# input arguments
# Example python3 model2dot.py -i XMLModelFile.xml -o out.gdot -f -c -n
    ...
    for i in range(tam):
       if(previousState[i].find(agent+":")==0 and previousState[i]==afterState[i]):
          ...
          while (j < tam):
             ...
             j = j+ 1
    for i in range(tam):
       if(previousState[i].find(agent+":")==0 and previousState[i].strip("\'")!=afterState[i].strip("\'")):
          afterState[i] = afterState[i].strip("\'")
...
try:
    opts, args = getopt.getopt(sys.argv[1:], "i:o:fcn", ["in=", "out="])
except getopt.GetoptError:
    print ('model2dot.py [-f] [-c] [-n] -i <model> -o <output>')
    print ('  -c : can have direct cycles')
    print ('  -f : outputs a source code skeleton of the functions used on the Model')
    print ('  -n : the agent will not be appended to the states names')
    sys.exit(' ')
for o, a in opts:
    if o in ("-i", "--in"):
        entrada = str(a)
...
for agent in agents:
   ...
   for childNode in (agent.getElementsByTagName('gpu:type')):
       ...
       if agentType in ['discrete','continuous']:
           break
   print ("agent: %s:%s" % (nome, agentType))
   ...
   primeiro = False
   file.write("subgraph cluster_%d{\n label=\"%s:%s\";color=blue; \n\n" % (clusterNumber,nome,agentType))
   ...
   if (not cycle):
      ...
      for k in range(len(funcOrder)):
          if(previousState[k].find(nome+":")==0 ):
              if apnameag:
                  file.write("   \"%s\" -> %s %s;\n" % (previousState[k].replace(':','_'), funcOrder[k], conditions[k] ))
                  file.write("   %s -> \"%s\";\n" %  (funcOrder[k],  afterState[k].replace(':','_')))
              else:
